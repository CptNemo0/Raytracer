...
namespace rtr
{
	...
	void Rasterizer::DrawMeshLightVertex(const mesh::ProceduralMesh& mesh)
	{
		...
		for (const auto& vertex : mesh.vertices_)
		{
			...
			for (const auto& light : lights_)
			{
				...
				color += light->CalculateColor(world_position, normal, preprocessor_->eye_position);
			}
	...
	void Rasterizer::DrawTriangleLightPixel(const mesh::Triangle& input, const math::ivec3 vidx, const std::vector<math::vec3>& positions, const std::vector<math::vec3>& normals)
	{
		const auto width = static_cast<float>(framebuffer_->Width());
		const auto height = static_cast<float>(framebuffer_->Height());
		auto& color_buffer = framebuffer_->ColorBuffer();
	...
	void Rasterizer::DrawMeshLightPixel(const mesh::ProceduralMesh& mesh)
	{
